...
一般而言,每一个线程都会有一个对应的`arena`,这么做的目的主要是为了减少内存分配时锁的争用,加快内存的分配与回收.

基本上,所有与内存分配相关的重要的结构体,都嵌入到了`arena`之中,比如`extent`,`tcache`,`base`分配器等,这些我们会在后面章节一一叙述.现在大家大致看一下即可.
...
struct arena_s {
	 ...
	 * Synchronization: internal.
	 */
	bins_t			bins[SC_NBINS]; /* bins用于存储空闲的内存块 */
