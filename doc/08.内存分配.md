...
arena_slab_alloc(tsdn_t *tsdn, arena_t *arena, szind_t binind, unsigned binshard,
	...
	if (slab == NULL && arena_may_have_muzzy(arena)) {
        /* 如果arena->extents_dirty上没有可用的extent,尝试从arena->extents_muzzy中复用extent */
...
```
为了减少分配的代价,我们总是优先重复使用之前已经释放的`extent`,如果成功找到了可以复用的`extent`,如果可以的话,需要重新将`extent`的内存标记为可读/可写,类似于重新向操作系统申请内存.

```c
/* 尝试复用extent
 * @param extents 可以从这个链表上查找extent来进行复用,注意这个extents可能为arena->extents_diry,arena->extents_muzzy
...
extent_recycle(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
	...
	if (*commit && !extent_committed_get(extent)) {
        /* 因为这里实际上是使用回收的extent,所以这里重新将extent所管理的内存标记为可读/可写 */
		if (extent_commit_impl(tsdn, arena, r_extent_hooks, extent,
		    0, extent_size_get(extent), growing_retained)) {
			extent_record(tsdn, arena, r_extent_hooks, extents, extent, growing_retained);
			return NULL;
		}
