...
我们来看一下,在没有`base`分配器的情况下,是如何分配内存的.
...
base_block_alloc(tsdn_t *tsdn, base_t *base, extent_hooks_t *extent_hooks,
	 ...
	 */
	size_t min_block_size = HUGEPAGE_CEILING(sz_psz2u(header_size + gap_size + usize)); /* 至少要分配这么多字节 */
	...
	if (metadata_thp_madvise()) { /* 透明大页,暂时忽略 */
		void *addr = (void *)block;
		assert(((uintptr_t)addr & HUGEPAGE_MASK) == 0 && (block_size & HUGEPAGE_MASK) == 0);
...
答案是直接使用`mmap`,注意下面调用的`extent_alloc_mmap`,可以去看一下`extent`这一节.
...
`base`分配器销毁的核心,是要销毁它所管理的内存块,也就是所谓的`base_block`.
...
关于`base_unmap`中的各个函数,可以查看`extent`章节,最终都是调用`munmap`来回收内存.
...
base_unmap(tsdn_t *tsdn, extent_hooks_t *extent_hooks, unsigned ind, void *addr, size_t size) {
	 ...
	 */
	if (extent_hooks == &extent_hooks_default) {
		if (!extent_dalloc_mmap(addr, size)) { /* 实际调用extent_dalloc_default */
			goto label_done;
		}
...
我们来看一下它是怎么来进行分配,`base`分配器分配的内存都来自`extent`,因此分配的第一步,就是找到一个合适的`extent`.

`base`分配器首先尝试从`base->avail[]`数组中获得一个和`size`大小相近的`extent`.
...
base_alloc_impl(tsdn_t *tsdn, base_t *base, size_t size, size_t alignment, size_t *esn) {
	...
	for (szind_t i = sz_size2index(asize); i < SC_NSIZES; i++) {
		extent = extent_heap_remove_first(&base->avail[i]); /* 取出一个extent,并将其从extent_heap中移除 */
		if (extent != NULL) {
			/* Use existing space. */
			break;
		}
...
如果没有从`base->avail[]`找到可用的`extent`,需要重新分配`extent`:
...
从`extent`分配完内存之后,`extent`可能还会存在剩余内存,为了尽可能利用完内存,`jemalloc`会将根据`extent`剩余内存大小,将`extent`再次放入`base->avail[]`数组,方便下一次的分配.
...
base_extent_bump_alloc_post(base_t *base, extent_t *extent, size_t gap_size,
	...
	if (extent_bsize_get(extent) > 0) { /* extent中还有可供分配的内存 */
		/*
		 * Compute the index for the largest size class that does not
		 * exceed extent's size.
		 */
		szind_t index_floor =
		    sz_size2index(extent_bsize_get(extent) + 1) - 1; /* 计算extent剩余内存的级别,注意这里减掉了1 */
        /* 将extent插入链表 */
